...
result_t KernelDriver::_runSystemCheck()
{
	...
	if (systemMgr.getSystemVersion() == OSVersion::OTHERS) {
		return unexpected_error("内核驱动模块在你的操作系统上不受支持。\n"
			                    "【注】驱动模块支持win7/8/8.1/10/11。", 0);
	}
	...
	if (systemMgr.getSystemVersion() == OSVersion::WIN_10_11 &&
		...
		if (!win11ForceEnable ||
			...
			if (systemMgr.messageBoxYesNo(strBsodAlert, "系统版本警告")) {
...
void KernelDriver::_importCertKey() {
	...
	if (status != ERROR_SUCCESS) {
		if (systemMgr.messageBoxYesNo("_importCertKey(): 创建注册表项失败，你可能需要手动安装证书。\n要打开证书下载页面么？", "注意")) {
...
result_t KernelDriver::_extractResource() {
	...
	if (rcSize <= 0) {
		return unexpected_error(__FUNCTION__ "(): SizeofResource失败。", GetLastError());
	}

	HGLOBAL hGlobal = LoadResource(NULL, hRsrc);
	if (hGlobal == NULL) {
		return unexpected_error(__FUNCTION__ "(): LoadResource失败。", GetLastError());
	}

	LPVOID rcBuf = LockResource(hGlobal);
	if (rcBuf == NULL) {
		return unexpected_error(__FUNCTION__ "(): LockResource失败。", GetLastError());
	}
